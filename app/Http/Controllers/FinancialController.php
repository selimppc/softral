...
use App\Http\Models\ProjectHours;
	...
	*/
	...
	public function addmoneycredit(Request $request){
		...
		if(isset($_POST['submit_credit'])):
			...
			$redirect='http://pronoor.com/shopping_cart?email='.$logged_user['email'].'&username='.$logged_user['email'].'&key='.$key_value.'&encrypt_pass='. urlencode($id1).'&amount='.$input['add_amount'].'&token='.$input['_token'].'&f_id='.$input['financial_accounts_id'];
	...
	public function termsandMilestone(){
		...
		if(isset($proposal->contract->approve_contract)&& $proposal->contract->approve_contract==1):
			Session::flash('error', 'Sorry, Your contract has been suspended.');
			return view('laravel-authentication-acl::client.exceptions.404');
		endif;
		
		if(isset($proposal->contract->hold_contract)&& $proposal->contract->hold_contract==0):
			Session::flash('error', 'Sorry, Your contract has been on hold.');
			return view('laravel-authentication-acl::client.exceptions.404');
		endif;
	...
	public function saveMilestone(Request $request){
		...
		$total_milestone = Milestone::where('job_id', '=', $input['job_id'])->where('proposal_id', '=', $input['proposal_id'])->count();
		
		if(isset($input['accept'])):
				$proposal = Proposal::where('id',$input['proposal_id'])->first();
				$job = Job::where('id',$input['job_id'])->first();
				$freelancer_email=$proposal['user']['email'];
				$proposal_id=$input['proposal_id'];
			if($input['accept']=='Accept'):
				$input1['employee_accept_contract']=1;
				Mail::send('emails.employee_accept_terms_milestone', ['label'=>'Employee','job' => $job,'proposal_id'=>$proposal_id,'proposal'=>$proposal,'accept'=>$input['accept']], function($message) use ($freelancer_email) {
					$message->to($freelancer_email, 'From Softral Job')->subject('Softral - Terms & Milestone notification');
				});
			else:
				$input1['employee_accept_contract']=0;
				Mail::send('emails.employee_accept_terms_milestone', ['label'=>'Employee','job' => $job,'proposal_id'=>$proposal_id,'proposal'=>$proposal], function($message) use ($freelancer_email) {
					$message->to($freelancer_email, 'From Softral Job')->subject('Softral - Terms & Milestone notification');
				});
			endif;
				$contract = Contract::findOrFail($input['contract_id']);
				$contract->fill($input1)->save();
		...
		if(isset($input['milestone_id']) && $input['milestone_id']!=''):
			...
			//echo $next_milestone->posted_date;exit;
			if(!isset($previous_milestone->posted_date)):
				if((isset($next_milestone->posted_date) && $input['posted_date']<$next_milestone->posted_date)):
					Session::flash('error', 'Your date can not be smaller then previous milestone date.');
					return redirect('financial/terms_milestone?p_id='.$input['proposal_id']);
				endif;
			elseif(!isset($next_milestone->posted_date)):
				if((isset($previous_milestone->posted_date) && $input['posted_date']<$previous_milestone->posted_date)):
					Session::flash('error', 'Your date can not be smaller then previous milestone date.');
					return redirect('financial/terms_milestone?p_id='.$input['proposal_id']);
				endif;
			else:
				...
				if(isset($last_get->amount) && $input['amount']>$last_get->amount):
				Session::flash('error', 'Your milestone exceeds by the total amount.');
				return redirect('financial/terms_milestone?p_id='.$input['proposal_id']);
				endif;
			
			if($total_milestone!=1):
		...
		if(isset($input['proposal_user'])):
		    $job = Job::where('id',$input['job_id'])->first();
			$employee_email=$job['user']['email'];
			$proposal_id=$input['proposal_id'];
			
			$input1['employee_accept_contract']=2;
		
			$contract = Contract::findOrFail($input['contract_id']);
			$contract->fill($input1)->save();
			
			Mail::send('emails.edit_terms_notification_employee', ['label'=>'Freelancer','job' => $job,'proposal_id'=>$proposal_id], function($message) use ($employee_email) {
				$message->to($employee_email, 'From Softral Job')->subject('Softral - Terms & Milestone notification');
			});
		else:
			$proposal = Proposal::where('id',$input['proposal_id'])->first();
			$job = Job::where('id',$input['job_id'])->first();
			...
			$input1['employee_accept_contract']=3;
		
			$contract = Contract::findOrFail($input['contract_id']);
			$contract->fill($input1)->save();
			
			Mail::send('emails.edit_terms_notification_employee', ['label'=>'Employee','job' => $job,'proposal_id'=>$proposal_id,'proposal'=>$proposal], function($message) use ($freelancer_email) {
				$message->to($freelancer_email, 'From Softral Job')->subject('Softral - Terms & Milestone notification');
			});
		endif;
	...
	public function saveEscrow(Request $request){
		...
		if(isset($input['deposit_for']) && $input['deposit_for']=='yes'):
			...
			if(!empty($money)):
					$total_amount=$money['total_amount']-$amount1-$freelancer_fee;
				else:
					$total_amount=$amount1;
			endif;
		
			$money_has = with(new Money)->escrowmoney_add($job_id,$logged_user->id,'escrow',$total_amount,$amount1);
			$softral_amount = Money::where('user_id',1)->orderBy('id','desc')->first();
			
			if(!empty($softral_amount)):
				$softral_amount=$softral_amount['total_amount']+$freelancer_fee;
			else:
				$softral_amount=$freelancer_fee;
			endif;
			
			$amount_to_softral = with(new Money)->add_escrow_release($input['job_id'],1,$softral_amount,$freelancer_fee,$logged_user->id);
		...
		elseif(isset($input['deposit_for']) && $input['deposit_for']=='no'):
			...
			$input['amount']=$input['amount']+$freelancer_fee;
			
			$redirect='http://pronoor.com/shopping_cart?email='.$logged_user['email'].'&username='.$logged_user['email'].'&key='.$key_value.'&encrypt_pass='. urlencode($id1).'&amount='.$input['amount'].'&token='.$input['_token'].'&escrow=1&f_id='.$input['financial_accounts_id'];
			return redirect($redirect);
		
		endif;

		if(isset($_POST['price'])):
			$amount1=$_POST['price']-$freelancer_fee;
			...
			Session::forget('financial_accounts_id_for_credit_shopping_cart');
			Session::flash('message', 'You have successfully deposit money to escrow. $'.$freelancer_fee.' has added to softral account as transaction fee');
		else:
			Session::flash('message', 'You have successfully deposited money to escrow.');
		endif;
		
			//$amount_to_proposal_user = with(new Money)->add_escrow_release($input['job_id'],$logged_user->id,$total_amount,$freelancer_fee,0);
			...
			$employee_email=$proposal['job']['user']['email'];
					...
					$input['job_id']=$job_id;
					$input['user_id']=$proposal['user_id'];
					$input['label']='Escrow money';
	...
	public function releaseEscrow(Request $request){
		...
		if($amount_to_softral>=$amount2):
				Session::flash('error', 'Sorry, You must release higher amount then Softral Freelancer Fee.');
				return redirect('financial/terms_milestone?p_id='.$proposal_id);
		endif;
		...
		if(!empty($softral_amount)):
			...
			Session::flash('message', 'You have successfully released amount from escrow. $'.$amount_to_softral.' was added to softral account as transaction fee');
	...
	public function releaseBonus(Request $request){
		...
		$softral_amount = Money::where('user_id',1)->orderBy('id','desc')->first();
			...
			if(!empty($money_freelancer)):
					$total_amount_freelancer=$money_freelancer['total_amount']+$amount1-$freelancer_amount['freelancer_fee'];
				else:
					$total_amount_freelancer=$amount1-$freelancer_amount['freelancer_fee'];
			endif;
	...
	public function saveTermsMilestone(Request $request){
		...
		if(isset($input['accept'])):
			...
			if($milestone['terms_milestone']==2):
				Session::flash('error', 'Sorry, This contract is declined by freelancer.');
				return redirect('financial/terms_milestone?p_id='.$milestone['id']);
			endif;
			
			if($input['accept']=='Decline offer'):
				$input['terms_milestone']=2;
				...
				Mail::send('emails.freelancer_declined_offer', ['proposal' => $milestone,'label'=>$label], function($message) use ($employee_mail) {
					$message->to($employee_mail, 'From Softral Job')->subject('Softral - Job Offer Notification');
				});
				
				$label='freelancer';
				Mail::send('emails.freelancer_declined_offer', ['proposal' => $milestone,'label'=>$label], function($message) use ($freelancer_email) {
					$message->to($freelancer_email, 'From Softral Job')->subject('Softral - Job Offer Notification');
				});
				
				Session::flash('error', 'You have successfully declined the offer.');
			...
			Session::flash('message', 'You have successfully approved the terms and condition. Please wait until Employee fund amount to Escrow.');
			return redirect('financial/terms_milestone?p_id='.$input['proposal_id']);
		else:
			$milestone = Proposal::findOrFail($input['proposal_id']);
	...
	public function editTerms(Request $request){
		...
		$freelancer_email=$contract['proposal']['user']['email'];
		$employee_mail=$contract['job']['user']['email'];
		
		$freelancer_name=$contract->proposal->user->user_profile[0]->first_name;
		$employer_name=$contract->job->user->user_profile[0]->first_name;
	
	
		Mail::send('emails.edit_terms_notification_both', ['contract' => $contract,'name'=>$employer_name], function($message) use ($employee_mail) {
				$message->to($employee_mail, 'From Softral Job')->subject('Softral - Terms Edited');
			});
			
		Mail::send('emails.edit_terms_notification_both', ['contract' => $contract,'name'=>$freelancer_name], function($message) use ($freelancer_email) {
				$message->to($freelancer_email, 'From Softral Job')->subject('Softral - Terms Edited');
			});
			
		Session::flash('message', 'You have successfully edited the terms.');
		return redirect('financial/terms_milestone?p_id='.$contract['proposal_id']);	
	}
	public function releaseShowhours()
    {
		...
		$projecthours = ProjectHours::where("job_id", $job_id)->where("proposal_id", $proposal_id)->where("contract_id", $contract_id)->orderBy('id','DESC')->get();
		...
		$itemsForCurrentPage = array_slice($projecthours->toArray(), $offSet, $perPage, true);
		$projecthours =  new LengthAwarePaginator($itemsForCurrentPage, count($projecthours), $perPage, $page);
		
		$html='';
		$html.= "<tr>";
			$html.= "<td><center><b>Date</b></center></td>";
			$html.= "<td><center><b>Hours</b></center></td>";
			$html.= "<td><center><b>Task</b></center></td>";
			$html.= "</tr>";
		
		foreach($projecthours as $projecthour)
			{
			
				$newDate = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $projecthour['date'])
                ->format('M d, Y');
			
			 $html.=  "<tr>";
			 $html.= "<td><center>".$newDate."</td>";
			 $html.= "<td><center>".$projecthour['hours']."</td>";
			 $html.= '<td align="center"><a href="javascript:void(0)" id="add_view_task_'  .  $projecthour['id']  .    '">View Task</a>';
			 $html.= '<div id="modal_release_view_task_' . $projecthour['id'] .  '" style="display: none" align = "center">'
					.$projecthour['task'].'</div>';
		
			$html.='<script>
				$(function () {
					
				$("#modal_release_view_task_' . $projecthour['id'] .  '").dialog({
            modal: true,
            autoOpen: false,
            title: "View Task",
            width: 300,
			...
			$("#add_view_task_'  .  $projecthour['id']  .    '").click(function () {
            $("#modal_release_view_task_' . $projecthour['id'] .  '").dialog("open");
			});
			});
			</script>';
			...
			return response()->json(['success' => true, 'projecthours' => $html]);		
}
	...
	public function TransactionsActivity(){
		$logged_user = $this->auth->getLoggedUser();
		$jobs = Job::where('user_id',$logged_user->id)->where('selected',1)->select('id')->get();
		...
		$money_transactions1 = Money::with('job','job.user.user_profile','user.user_profile')->where('user_id',$logged_user->id)->orderBy('id','asc')->get();
		$money_transactions2 = Money::with('job','job.user.user_profile','user.user_profile')->where('user_id','!=',$logged_user->id)->whereIn('job_id', $job_array)->orderBy('id','asc')->get();
		$money = Money::where('user_id',$logged_user->id)->orderBy('id','desc')->first();
		
			
		$money_transactions = array_merge($money_transactions1->toArray(), $money_transactions2->toArray());
		$money_transactions = array_values(array_sort($money_transactions, function ($value) {
			return $value['id'];
		}));
        ...
        $collection = new Collection(array_reverse($money_transactions));
		...
		return View::make('laravel-authentication-acl::admin.financial.financialActivity')->with([ 'money'=>$money,'all_transactions'=>($paginatedSearchResults),'user_id'=>$logged_user->id]);
	}
	...
	public function acceptMilestone(Request $request){
		...
		if(!empty($softral_amount)):
			...
			$escrow_has = with(new Escrow)->add_deposit($job_id,$proposal_id,$amount);
			$escrow_has = with(new Money)->add_escrow_release($job_id,$escrow_money['proposal']['user_id'],$total_amount,$amount1,0);
			$amount_to_softral1 = with(new Money)->add_escrow_release($job_id,1,$softral_amount,$amount_to_softral,$escrow_money['proposal']['user_id']);
		...
		Session::flash('message', 'You have successfully accepted the milestone. $'.$amount_to_softral.' was added to softral account as transaction fee');
