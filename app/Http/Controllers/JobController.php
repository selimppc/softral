...
class JobController extends Controller {
	...
	*/
    ...
    // start count how many uploaded
		...
		$input=Job::create($input);
		
		$email=array();
		foreach($input_skill_id as $key=>$value)
		{
			$email_array= ProfileField::with('user_profile')->where('profile_field_type_id', 3)->where('value','REGEXP','.*s:[0-9]+:"'.$value.'"*')->get();
			foreach($email_array as $emails):
				if($emails['user_profile']['user']['email']!=''):
					$email[]=$emails['user_profile']['user']['email'];
					$name[]=$emails['user_profile']['first_name'];
				endif;
			endforeach;
	...
	public function AddProposal($job=null,$id=null)
	{
		...
		if($logged_user->user_profile[0]->profile_field_type->value!='Seller' && $logged_user->user_profile[0]->profile_field_type->value!='Both'):
			...
			$job_detail = Job::with('categories.parent_get','user','children')->where('slug', $job)->where('job_close', '!=',2)->first();
			
			if(empty($job_detail)):
					Session::flash('message', 'This job has been closed.');
					return view('laravel-authentication-acl::client.exceptions.404');
			endif;
			$exist_proposal = Proposal::where('user_id', $logged_user->id)->where('job_id', $job_detail->id)->first();
			
			if($job_detail['status']==0):
				Session::flash('error', 'Sorry, This job is been inactive due to some reason.');
				return view('laravel-authentication-acl::client.exceptions.404');
			endif;
		...
		if($job_detail->selected==1):
			...
			try
			{
				...
				$proposal_suspend = Proposal::where('status',0)->find($id);
				$milestone = Milestone::where('proposal_id',$id)->first();
				
				if(!empty($proposal_suspend)):
					Session::flash('message', 'Your proposal is temporary suspended.');
					return view('laravel-authentication-acl::client.exceptions.404');
				endif;
	...
	public function SaveProposal(Request $request)
	{
			...
			$input['amount']['charged_client']=$input['amount']['paid_to']+$freelancer_amount['employee_fee'];
			$amount=json_encode($input['amount']);
			$amount_array=$input['amount'];
			
			$input['user_id']= $logged_user->id;
			$input['amount']= $amount;
			$job_detail = Job::where('id', $input['job_id'])->where('job_close', '!=',2)->first();
			...
			if(Input::get('id')):
				...
				Milestone::where('proposal_id', Input::get('id'))->update(['posted_date' => $amount_array['duration'],'amount'=>$amount_array['paid_to']]);
				...
				$input['amount']= $amount_array['paid_to'];
	...
	public function deletesaveJob(){
		...
		if(Input::get('save_user_id')):
			$savejob = Savejob::where('user_id', Input::get('user_id'))->where('save_user_id', Input::get('save_user_id'))->delete();
		else:
			$savejob = Savejob::where('user_id', Input::get('user_id'))->where('job_id', Input::get('job_id'))->delete();
		endif;
		//$savejob->delete();
		Session::flash('message', 'You have successfully removed item!');
 
		if(Input::get('save_user_id')):
			return redirect()->route('welcome.saveUsers');	
		else:
	...
	public function closeJob(){
		$logged_user = $this->auth->getLoggedUser();
		$job_detail = Job::where('id', Input::get('job-id'))->first();
	...
	public function openJob(){
		...
		$job_close = DB::table('jobs')->where('id', Input::get('job-id'))->update(array('job_close' => 0,'selected' => 0));
	...
	public function Selectproposal(Request $request){
		$input = $request->all();
		
		$proposal_detail = Proposal::with('job','user')->where('id', $input['id'])->where('offer', 0)->first();
		$proposal_suspend = Proposal::with('job','user')->where('id', $input['id'])->where('status', 0)->first();
		$proposal = DB::table('proposals')->where('id', $input['id'])->update(array('offer' => 1));
		
		if(isset($input['close_job']) && $input['close_job']==1):
			$job = DB::table('jobs')->where('id', $input['job_id'])->update(array('selected' => 1,'job_close' => 2));
		else:
			$job = DB::table('jobs')->where('id', $input['job_id'])->update(array('selected' => 1));
		endif;
		
		$logged_user = $this->auth->getLoggedUser();
		$job_detail = Job::where('id', $input['job_id'])->first();
	
		if(($logged_user->id!=$job_detail->user_id) || empty($proposal_detail)):
			return view('laravel-authentication-acl::client.exceptions.404');
		endif;
		...
		$input1['job_id']=$input['job_id'];
		$input1['proposal_id']=$input['id'];
		$contract=Contract::create($input1);
		...
		if($proposal_detail['freelancer_counter_amount']!=''):
			...
			$input_amount['paid_to']=$proposal_detail['freelancer_counter_amount']-$freelancer_fee;
			//$input_amount['softral_fee']=$proposal_detail['freelancer_counter_amount']*5/100;
			$input_amount['charged_client']=$proposal_detail['freelancer_counter_amount'];
			$input_amount['duration']=$amount->duration;
			$amount=json_encode($input_amount);
			
			Milestone::where('proposal_id', $input['id'])->update(['amount' => $input_amount['paid_to']]);
			Proposal::where('id', $input['id'])->update(['amount' => $amount]);
		endif;
		
				$notification['label'] = 'select proposal';
				$notification['job_id'] =Input::get('job_id');
				$notification['user_id'] =$proposal_detail['user_id'];
				$notification['proposal_id'] =Input::get('id');
				$notification = Notifications::create($notification);
		...
		return redirect('financial/terms_milestone?p_id='.$input['id']);
	}
	...
	public function viewProposal(){
		...
		$proposal_suspend = Proposal::with('job','user_profile.profile_field','contract')->where('id', Input::get('id'))->where('status', 0)->first();
		if(empty($proposal)):
			return view('laravel-authentication-acl::client.exceptions.404');
		endif;
	...
	public function Jobdetail($job=null)
	{
		...
		if($job_exist['status']==0):
			Session::flash('error', 'Sorry, Your job is been inactive due to some reason.');
			return view('laravel-authentication-acl::client.exceptions.404');
		endif;
		...
		$proposal_selected = Proposal::where("job_id", $job_detail['id'])->where("offer", 1)->orderBy('id', 'desc')->first();
		//dd($job_detail);exit;
	...
	public function savejob($job_id='',$type=''){
        //PDF file is stored under project/public/download/info.pdf
       
		$logged_user = $this->auth->getLoggedUser();
		
		if($type=='save_user'):
			$input['save_user_id']=$job_id;
		else:
			$input['job_id']=$job_id;
		endif;
